...
const templatePath = `${process.argv[1].replace("/.bin", "")
    ...
    .replace(/\/generate-next-routes-list\/*.*/g, "/next-routes-list")}/dist/routes.js`;
const templateString = node_fs_1.default.readFileSync(templatePath, "utf-8");
const newTemplateString = templateString.replace("const routes = [];", `const routes = ${JSON.stringify(routes)};`);
node_fs_1.default.writeFileSync(templatePath, newTemplateString);
